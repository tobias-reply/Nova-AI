...
- Send call to Nova Lite (https://docs.aws.amazon.com/nova/latest/userguide/getting-started-api.html)
- How to send images (https://docs.aws.amazon.com/nova/latest/userguide/prompting-vision-prompting.html)
...
I want to create a modern interface that uses the users webcam. Every 30 seconds the system will take a screenshot of the webcam and send it to Amazon Bedrock with the prompt: describe the picture. The prompt will always stay the same so we can hardcode it. The returning text will then be displayed.
...
This example uses another model. We want to use the NovaLite Model with which we can interact like this. For the message simply use the phrase "Describe the picture": 
import boto3
import json
