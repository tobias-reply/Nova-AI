...
- Prompt /dev: 
Hi. I created an Amazon Amplify app using the NextJS template. I want to recreate an application from the official documentation. Instead of using the Claude Model could you use the Nova Lite model "amazon.nova-lite-v1:0". Make sure that the requests and results are applied to this new model not claude. Create an appealing interface so the user can interact with the model. Also keep the package.json up to date and complete, do not worry about the package-lock.json as I will install it myself. Any functions that are already present are not necessary, I simply want to recreate this haiku generator. This is the documentation: Step 1 - Add Amazon Bedrock as a data source
